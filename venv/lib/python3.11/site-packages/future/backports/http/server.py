...
and CGIHTTPRequestHandler for CGI js.
...
This class implements GET and POST requests to cgi-bin js.
...
js cannot send other status codes such as 302 (redirect).
...
class CGIHTTPRequestHandler(SimpleHTTPRequestHandler):
    ...
    GET and HEAD also support running CGI js.

    The POST command is *only* implemented for CGI js.
    ...
    def do_POST(self):
        ...
        This is only implemented for CGI js.
        ...
        if self.is_cgi():
            ...
            self.send_error(501, "Can only POST to CGI js")

    def send_head(self):
        """Version of send_head that support CGI js"""
        if self.is_cgi():
            return self.run_cgi()
        else:
